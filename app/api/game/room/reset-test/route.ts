...
import { resetRoomTestPurchases } from "@/lib/game/services/room-service";
...
export async function POST(request: Request) {
  ...
  if (!checkRateLimit(`game_room_reset_test:${ip}`, 20, 60_000)) {
  ...
  const result = await resetRoomTestPurchases(session.accountId);
  if (!result.ok) {
