...
export async function POST(
  ...
  if (!checkRateLimit(`game_runs_retreat:${ip}`, 120, 60_000)) {
  ...
  let body: RetreatBody;
  try {
    body = (await request.json()) as RetreatBody;
  } catch {
  ...
  const result = await retreatRunScene(session.accountId, runId, sceneId);
  if (!result.ok) {
