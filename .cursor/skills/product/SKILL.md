...
- **Main menu** offers **Continue** into the chapter map (**Gioca**), **Negozio** (furnish the exchange-room with pizza slices), and **Classifica** (rankings are optional motivation, not required to progress).
- **Shop (Negozio):** same hub chrome as chapter lists (back to menu, **pizza + backpack** in the header). Header pizza shows **spendable balance**; buying room items debits that balance only—the **leaderboard** still counts everything they earned in quests.
...
- **Leaderboard:** compare progress by **total pizza earned in quests** (not what is left after shop)—**Overall** (all learners) or **Teams** (blue vs red). Shop spending does **not** lower rank. Players can **refresh** after playing; their own row should be easy to find without shaming low scores.
...
| ---------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Pizza slices** | **Performance**—how well they did on a **task** (or **bonus**) step. Awarded **variably** from authored **scoring** rules (e.g. more correct answers → more slices). Same answers should always yield the same slices (**fair**, server-side). **Header / shop** show **spendable** pizza (earned minus room purchases). **Leaderboard** ranks by **lifetime earned**—shop does not reduce the score. Backpack is separate and unchanged by the shop. |
