...
- **Row layout (sentence-style):** `.lg-cloze-line-row` uses **`flex-wrap: nowrap`**, **`align-items: center`**, **`overflow-x: auto`**—not `flex-wrap: wrap` (that pattern is for Error Spotting chip rows). Dense dialogue: more **`lines[]` rows**, fewer gaps per row (`20260628160000_chapter_02_nutelleria_cloze_lines.sql`).
