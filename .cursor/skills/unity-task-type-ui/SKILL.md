...
- **Avatar beats:** `innerMonologue` / `npcDialog` use **`CutsceneInnerMonologueBeat.uxml`** / **`CutsceneNpcDialogBeat.uxml`** — stable anchors **`avatar-slot`**, **`bubble-col`**, **`beat-body`** (and **`npc-name`** on NPC). Option B Italian fixtures in UXML; on bind, **`CutsceneToolkitStep`** overwrites labels and **`CutsceneAvatarSlotBinder.BindPlayerSlot` / `BindNpcSlot`** after **`ClearHost(avatar-slot)`**. Portraits: **`GameArt/portraits/player/current`** and **`GameArt/portraits/npc/{portraitId}`** via **`GameArtResourceLoader`** (legacy **`CutscenePortraits`** fallback). Do not drive row order from **`npcCast.side`** (fixed NPC-right layout). USS: **`cutscene-narrative.uss`** (`lg-cutscene-beat-row`, placeholders).
