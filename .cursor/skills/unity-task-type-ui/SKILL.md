...
**Scene backgrounds (shell / navigation UXML):** Root `lg-scene-bg-root lg-gameart-bg--{screen}` + child `scene-background-host` (`lg-scene-background-host`) as **sibling of `main-column` on the shell root** (task and cut shells); content column uses **`lg-fill-screen--transparent`** (not `lg-fill-screen`). Do not place the host only inside `cut-shell-stage`—`BindFromContentJson` clears the root and full-bleed art will clip to the stage. USS previews in `game-art-backgrounds.uss`; runtime via **`ToolkitNavigationScreenBinder`** (nav) or **`BindFromContentJson`** (per-step JSON on host only). New navigation screen: add `lg-gameart-bg--*` rules in USS, keys in **`GameArtAssetKeys`**, call `Apply*Screen` in the view `Awake`. Theme: **`LearningMenusTheme.tss`** / **`LearningMenusPanelSettings`** for UI Builder.
...
**`contentJson`** / rewards / cutscenes: [`docs/authoring/02-steps-and-rewards.md`](../../docs/authoring/02-steps-and-rewards.md).
...
- **Row layout (sentence-style):** `.lg-cloze-line-row` uses **`flex-wrap: wrap`**, **`align-items: center`**, labels **`flex-shrink: 0`** (never `nowrap` + shrink — long single-line cloze with many gaps, e.g. chapter-01 vacation, collapses into one-character columns). Tall cloze tasks scroll via **`cloze-lines-scroll`** (`ScrollView`, vertical). Dense dialogue: prefer more **`lines[]` rows** when authoring (`20260628160000_chapter_02_nutelleria_cloze_lines.sql`), but wrap must still work for legacy one-line payloads.
- Gap fields: **`ClozeGapFieldPart.uxml`** with **`lg-cloze-gap-inline`** (not full-height **`lg-textfield`** form fields); **`margin-bottom: 0`** on inline gaps. Content host: **`lg-cloze-lines-host`** (`flex-grow: 1`, `min-height: 0`) inside the task template.
