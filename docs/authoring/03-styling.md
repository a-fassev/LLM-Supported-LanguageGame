...
Recurring map / shell header chrome lives in **`Templates/Parts/Navigation/`** and is composed into `*Screen.uxml` with **`ui:Template` + `ui:Instance`** (same pattern as task templates — see [`.cursor/skills/unity-task-type-ui/SKILL.md`](../../.cursor/skills/unity-task-type-ui/SKILL.md)):
