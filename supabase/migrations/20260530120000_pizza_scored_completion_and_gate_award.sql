-- Scored pizza + server-trusted slice awards via p_awarded_slices.
-- - pizza.mode = 'flat'  -> unchanged (value from reward_rules, ignores p_awarded_slices)
-- - pizza.mode = 'scored' -> awards clamped p_awarded_slices to 0..maxSlices (maxSlices capped 0..5 in-RPC)
-- Freitext LLM gates store pizza_slices_award at evaluation time for scored pizza.

alter table public.player_freitext_llm_gates
add column if not exists pizza_slices_award integer not null default 0;

comment on column public.player_freitext_llm_gates.pizza_slices_award is
  'Integer pizza slices to grant on /complete when reward_rules.pizza.mode=scored (server-computed at /evaluate).';
...
begin
  ...
  v_mode := lower(trim(coalesce(v_pizza ->> 'mode', '')));

  if v_mode = '' and coalesce(trim(v_pizza ->> 'value'), '') <> '' then
    v_mode := 'flat';
  end if;

  if v_mode = 'scored' then
    begin
      v_raw := trim(coalesce(v_pizza ->> 'maxSlices', v_pizza ->> 'max_slices', ''));
      if v_raw = '' then
        v_max_cap := 5;
      else
        v_max_cap := round(v_raw::numeric)::int;
      end if;
      if v_max_cap < 0 then v_max_cap := 0; elsif v_max_cap > 5 then v_max_cap := 5; end if;
      v_awarded_slices := p_awarded_slices;
      if v_awarded_slices < 0 then v_awarded_slices := 0;
      elsif v_awarded_slices > v_max_cap then v_awarded_slices := v_max_cap;
      end if;
    exception when others then
      v_awarded_slices := 0;
    end;
  elsif v_mode = 'flat' then
    begin
