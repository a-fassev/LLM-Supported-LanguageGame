...
- **Wallet HUD (pizza + backpack pieces):** Displayed via UI Toolkit where the screen defines wallet chips (**ChapterOverview**, **task quest shell**, **AvatarShop**). **Cutscene steps hide the wallet** (story mode). **`MainMenu` may omit wallet labels** while bootstrap still refreshes session totals for later scenes.
- **Leaderboard screen:** `MainMenuScreen.uxml` exposes **`leaderboard-button`** (under **Continue**). `LeaderboardView` + `LeaderboardScreen.uxml` use the same **UI Builder fixture** pattern as quest steps: sample rows under `leaderboard-list-host` (`lg-preview-sample`), then **`ToolkitStepUx.ClearHost`** + `Templates/Parts/LeaderboardPlayerRowPart.uxml` via `ToolkitLeaderboardUx` at runtime. **Overall** lists all players by pizza total; **Teams** shows blue/red team totals plus the player list. Data from **`GET /api/game/leaderboard`** (initial load on open; **`refresh-button`** re-fetches in place). Pause chrome returns to main menu (`LearningToolkitPauseChromeBinder`).

**Quest scene shell routing (one `Quest` scene, two shells):**

- **`QuestStepShellHost`** (`Assets/Scripts/Presentation/QuestStepShellHost.cs`) mounts **exactly one** shell per refresh: **`TaskShellPresenter`** + `TaskShellScreen.uxml` or **`CutsceneShellPresenter`** + `CutShellScreen.uxml`. Switching `step_kind` is a **UIDocument teardown/mount in the same scene**—not separate `Task`/`Cut` scenes.
